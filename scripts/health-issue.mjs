...
/* "Published" here means the catalog has a verified photo for it, which is the
   same condition server.js uses to serve a row. */
const served = new Set(images.filter((r) => r.have).map((r) => r.id));
const BROKEN = new Set(['LISTING', 'HOME', 'DEAD', 'PARKED', 'GROUP']);

const badLinks = links.filter((r) => served.has(r.id) && BROKEN.has(r.cls));
const badImages = images.filter((r) => r.cls === 'MISMATCH');
